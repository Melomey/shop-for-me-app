        ...
        {items.map(item => (
          <div key={item.id} className="relative mb-4 p-4 border border-gray-300 rounded-md">
            ...
            <div className="flex justify-end mt-2">
              <button onClick={() => deleteItem(item.id)} className="mr-2 px-4 py-2 bg-red-500 text-white rounded-md">Delete</button>
              <button onClick={() => editItem(item.id)} className="px-4 py-2 bg-blue-500 text-white rounded-md">Edit</button>
            </div>
          </div>
        ))}
      </div>
...
export default MakeItemListContent;
